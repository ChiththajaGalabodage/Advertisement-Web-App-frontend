...
export default function MobilePhone() {
  ...
  const fetchListings = useCallback(async () => {
    try {
      ...
      results = results.filter(
        (listing) => listing.category?.toLowerCase() === "mobile phone",
      );
  ...
  function handleSearch(e) {
    e.preventDefault();
    if (searchInput.trim()) {
      navigate(`/search?q=${searchInput.trim()}&category=Mobile Phone`);
    }
  }
    ...
    <>
      ...
      <div className="flex-1 w-full py-8 px-4 bg-gray-50">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="mb-6">
            <h2 className="text-3xl font-bold text-gray-800">Mobile Phone</h2>
            <p className="text-gray-600">Found {listings.length} listing(s)</p>
          </div>
