...
export default function Education() {
  ...
  const fetchListings = useCallback(async () => {
    try {
      ...
      results = results.filter(
        (listing) => listing.category?.toLowerCase() === "education",
      );
      if (minPrice)
        results = results.filter(
  ...
  function handleSearch(e) {
    e.preventDefault();
    if (searchInput.trim()) {
      navigate(`/search?q=${searchInput.trim()}&category=Education`);
    }
  }
        ...
        <div className="max-w-6xl mx-auto">
          ...
          {!loading && !error && listings.length === 0 && (
            <div className="text-center py-12">
              <p className="text-gray-600 mb-4">No Education listings found.</p>
              <button
